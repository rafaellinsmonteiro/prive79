...
const ModelOnlineToggle = () => {
  ...
  const loadOnlineStatus = async () => {
    ...
    try {
      const {
        data: modelData,
        error
      } = await supabase.from('models').select('is_online, manual_status_override').eq('id', model.id).single();
  ...
  const toggleOnlineStatus = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: newOnlineStatus ? "Você aparecerá como disponível para clientes" : "Você não aparecerá como disponível para clientes"
      });
    } catch (error) {
  ...
  return <div className="flex items-center gap-3 p-3 border rounded-lg bg-background">
      <div className="flex items-center gap-2">
        {isOnline ? <Wifi className="w-4 h-4 text-green-500" /> : <WifiOff className="w-4 h-4 text-gray-500" />}
        <Switch checked={isOnline} onCheckedChange={toggleOnlineStatus} disabled={loading} />
