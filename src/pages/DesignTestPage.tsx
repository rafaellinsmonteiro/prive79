...
import { LayoutDashboard, Users, MessageSquare, Calendar, TrendingUp, Settings, LogOut, Sun, Moon, ChevronLeft, Search, Bell, Star, Heart, BarChart3, PlusCircle, Filter, Download, Zap, Menu } from 'lucide-react';
...
const DesignTestPage = () => {
  ...
  return <div className={`min-h-screen flex w-full ${isDark ? 'dark' : ''}`}>
        ...
        <div className="flex-1 px-4 py-6 space-y-8">
          ...
          <div>
            {(isExpanded || isMobileMenuOpen) && (
              <div className="text-xs font-semibold mb-4 px-3 text-[hsl(var(--sidebar-text-muted))] uppercase tracking-wider">
                Principal
              </div>
            )}
      ...
      {/* Main Content */}
      <div className={`flex-1 bg-[hsl(var(--content-bg))] overflow-hidden ${isMobile ? 'w-full' : ''}`}>
        {/* Header */}
        <header className="bg-[hsl(var(--header-bg))] border-b border-[hsl(var(--sidebar-border))] px-4 lg:px-8 py-4 lg:py-6 shadow-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              {isMobile && (
                <Button 
                  ...
                  className="p-2 hover:bg-[hsl(var(--sidebar-hover))]"
                >
                  <Menu className="w-5 h-5 text-[hsl(var(--sidebar-text-muted))]" />
                </Button>
              )}
              
              <div>
                <h1 className="text-xl lg:text-3xl font-bold text-[hsl(var(--sidebar-text))] mb-1">
                  Dashboard
                </h1>
                <p className="text-sm lg:text-base text-[hsl(var(--sidebar-text-muted))] hidden sm:block">
            ...
            <div className="flex items-center gap-2 lg:gap-4">
              ...
              {!isMobile && (
                <div className="relative hidden md:block">
                  ...
                  <Input placeholder="Buscar..." className="pl-10 w-60 lg:w-80 bg-[hsl(var(--sidebar-hover))] border-[hsl(var(--sidebar-border))] focus:border-primary" />
                </div>
              )}
              
              <Button variant="ghost" size="sm" className="relative p-2 hover:bg-[hsl(var(--sidebar-hover))]">
                <Bell className="w-4 h-4 lg:w-5 lg:h-5 text-[hsl(var(--sidebar-text-muted))]" />
                <div className="absolute -top-1 -right-1 w-2 h-2 lg:w-3 lg:h-3 bg-primary rounded-full"></div>
              </Button>
              
              <Button className="bg-gradient-to-r from-primary to-yellow-500 hover:from-primary/90 hover:to-yellow-500/90 text-primary-foreground shadow-[var(--shadow-gold)] text-sm lg:text-base px-3 lg:px-4">
                <PlusCircle className="w-3 h-3 lg:w-4 lg:h-4 mr-1 lg:mr-2" />
                <span className="hidden sm:inline">Novo</span>
                <span className="sm:hidden">+</span>
              </Button>
              
              <div className="flex items-center gap-2 lg:gap-3 pl-2 lg:pl-4 border-l border-[hsl(var(--sidebar-border))]">
                <Avatar className="w-8 h-8 lg:w-10 lg:h-10 ring-2 ring-primary/20">
                  ...
                  <AvatarFallback className="bg-gradient-to-br from-primary to-yellow-500 text-primary-foreground font-semibold text-xs lg:text-sm">
                    JW
                  </AvatarFallback>
                </Avatar>
        ...
        <main className="p-4 lg:p-8 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-6 mb-6 lg:mb-8">
            {stats.map((stat, index) => (
              <Card key={index} className="bg-[hsl(var(--sidebar-bg))] border-[hsl(var(--sidebar-border))] hover:shadow-lg transition-all duration-200 group">
                <CardContent className="p-3 lg:p-6">
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                      <p className="text-xs lg:text-sm text-[hsl(var(--sidebar-text-muted))] mb-1 truncate">{stat.label}</p>
                      <p className="text-lg lg:text-2xl font-bold text-[hsl(var(--sidebar-text))] truncate">{stat.value}</p>
                      <p className="text-xs lg:text-sm text-primary font-medium">{stat.change}</p>
                    </div>
                    <div className="p-2 lg:p-3 bg-gradient-to-br from-primary/10 to-yellow-500/10 rounded-xl group-hover:scale-110 transition-transform duration-200 ml-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-8">
            ...
            <Card className="lg:col-span-2 bg-[hsl(var(--sidebar-bg))] border-[hsl(var(--sidebar-border))]">
              <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <CardTitle className="text-[hsl(var(--sidebar-text))] text-lg lg:text-xl">Análise de Performance</CardTitle>
                <div className="flex gap-2 w-full sm:w-auto">
                  <Button variant="outline" size="sm" className="border-[hsl(var(--sidebar-border))] flex-1 sm:flex-none">
              ...
              <CardContent>
                <div className="h-60 lg:h-80 bg-gradient-to-br from-primary/5 to-yellow-500/5 rounded-xl flex items-center justify-center">
                  <div className="text-center">
                    <BarChart3 className="w-12 h-12 lg:w-16 lg:h-16 text-primary mx-auto mb-4" />
                    <p className="text-sm lg:text-base text-[hsl(var(--sidebar-text-muted))]">Gráfico de análise seria exibido aqui</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-[hsl(var(--sidebar-bg))] border-[hsl(var(--sidebar-border))]">
              <CardHeader>
                <CardTitle className="text-[hsl(var(--sidebar-text))] flex items-center gap-2 text-lg lg:text-xl">
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  time: '15 min atrás'
                    ...
                    <div key={index} className="flex items-center gap-3 p-2 lg:p-3 rounded-lg hover:bg-[hsl(var(--sidebar-hover))] transition-colors duration-200">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs lg:text-sm text-[hsl(var(--sidebar-text))] truncate">
