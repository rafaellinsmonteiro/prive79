...
const DesignTestPage = () => {
  ...
  const overviewItems = [
    ...
    { icon: BarChart3, label: 'Static', id: 'static' },
  ];
  ...
  return (
    <div className={`min-h-screen flex w-full ${isDark ? 'dark' : ''}`}>
      {/* Sidebar */}
      <div className={`${isDark ? 'bg-gray-900' : 'bg-white'} border-r transition-all duration-300 ${
        ...
        <div className="p-4 border-b flex items-center justify-between">
          ...
          <Button
            ...
            className={`${isDark ? 'text-gray-400 hover:text-white' : 'text-gray-600 hover:text-gray-900'} p-1`}
          >
        ...
        <div className="flex-1 px-3 py-4">
          ...
          <div className="mb-6">
            {isExpanded && (
              <div className={`text-xs font-medium mb-3 px-3 ${isDark ? 'text-gray-400' : 'text-gray-500'} uppercase tracking-wider`}>
                Overview
              </div>
            )}
                ...
                >
                  <Button
                    ...
                    className={`w-full justify-start px-3 py-2 h-10 ${
                      item.active
                        ? `${isDark ? 'bg-teal-900 text-teal-400' : 'bg-teal-50 text-teal-600'}`
                  ...
                  >
                    ...
                    {isExpanded && (
                      <>
                        ...
                        {item.badge && (
                          <Badge variant="secondary" className="ml-auto bg-teal-500 text-white text-xs">
                            {item.badge}
                          </Badge>
                        )}
                      </>
                  ...
                  {!isExpanded && hoveredItem === item.id && (
                    <div className="absolute left-16 top-2 z-50 px-2 py-1 text-sm bg-gray-800 text-white rounded shadow-lg">
                      {item.label}
                    </div>
                  )}
                </div>
