...
-- Atualizar a constraint para permitir que itens filhos não tenham URL nem categoria obrigatórias
-- (apenas itens raiz precisam ter conteúdo)
...
ALTER TABLE public.menu_items ADD CONSTRAINT check_menu_item_data CHECK (
  -- Itens filhos podem não ter URL nem categoria (apenas título)
  (parent_id IS NOT NULL) OR 
  -- Itens pai devem ter URL ou categoria
