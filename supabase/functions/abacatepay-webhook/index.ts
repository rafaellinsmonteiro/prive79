...
serve(async (req) => {
  ...
  try {
    ...
    // Verificar se é um evento de PIX pago
    if (body.event === 'pixQrCode.paid' && body.data) {
      const pixId = body.data.id;
      const status = body.data.status;
      const amount = body.data.amount / 100; // AbacatePay envia em centavos
